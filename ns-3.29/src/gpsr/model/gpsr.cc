...
RoutingProtocol::RecvGPSR (Ptr<Socket> socket)
{
  ...
  //屏蔽掉部分的gps位置信号
  if(!m_neighbors.HasValidPosition(Position)){
      //std::cout << "pass position: " << Position.x << " : " << Position.y << "\n";
  }
  else{
  ...
  //通过y值判断究竟是不是一个测线的
  T_Connect recv;
  recv.connectID = hdr.selfID;
  
  Vector myPos;
  Ptr<MobilityModel> MM = m_ipv4->GetObject<MobilityModel> ();

  if(Position.y != MM->GetPosition ().y){
    return;
  }
  //是的话，更新一下table
  //首先构建出参数
  recv.count = hdr.csCount;
  for(int i = 0; i < (int)recv.count; i++){
    recv.cs[i].id = hdr.cs[i].id;
    recv.cs[i].p = hdr.cs[i].p;
  }
  
  uint32_t lastRecv[MAX_CS_COUNT];
  for(int i = 0; i < (int)hdr.recvCount; i++){
    lastRecv[i] = hdr.recvList[i];
  }
  
  m_neighbors.AddRecv(&recv, recv.count, lastRecv, hdr.recvCount);
}
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      helloHeader.selfID = m_neighbors.selfID;
      //获取CS，并填写到hello数据包中, 主要是cs集合和上一次收到的集合
      int wIndex = 0;
      for (int i = 0; i < MAX_CS_COUNT; i++)
      {
        if(m_neighbors.cs[i].connectID != NO_CONNECT){
          helloHeader.cs[wIndex].id = m_neighbors.cs[i].connectID;
          helloHeader.cs[wIndex++].p = m_neighbors.cs[i].p;
        }
      }
      helloHeader.csCount = wIndex;
      //获取接受列表，然后填写进去
      wIndex = 0;
      for(int i = 0; i < MAX_CS_COUNT; i++){
        if(m_neighbors.lastRecv[i] != NO_CONNECT){
          helloHeader.recvList[wIndex++] = m_neighbors.lastRecv[i];
        }
      }
      helloHeader.recvCount = wIndex;
...
//关键函数： 转发，转发的流程是在这里里面。需要找到后一个节点，检车是不是在同一条线上，不过不是
//那么就触发FD模式。
bool
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  //std::cout << m_neighbors.selfID << " Recv from " << tHeader.srcID << std::endl;

  if (!tHeader.IsValid ())
  {
    std::cout << "valid tHeader! \n";  
  }
    ...
    //std::cout << m_neighbors.selfID << "(" << m_neighbors.selfID - 255 << ") GPSR To" << nextHop << std::endl;

    int EnableFD = 1;

    if(EnableFD == 1){
      if(m_locationService->GetPosition(nextHop).y != myPos.y){
        //获取到CS
        T_Connect his;
        his.count = 0;
        for(uint32_t i = 0; i < MAX_CS_COUNT; i++){
          his.cs[i].id = NO_CONNECT;
        }
        m_neighbors.getAllCsWithPosition(&his); //得到同一条测线上有位置信息的节点，作为历史使用.
        m_neighbors.addCurCs(&his);

        //计算后续的节点
        Ipv4Address nextTarget = m_neighbors.GetNextFDNode(&his);   

        if(nextTarget == Ipv4Address::GetZero ()){
          //遵守原来的转发策略
        }
        else{
          //std::cout << m_neighbors.selfID << "(" << m_neighbors.selfID - 255 << ") To FD To: " << nextTarget << "\n";
          FDHeader fdHeader1;
          uint32_t hisCount = 0;
          for(uint32_t i = 0; i < MAX_CS_COUNT; i++){
            if(his.cs[i].id != NO_CONNECT && his.cs[i].jump <= 3){
              fdHeader1.idList[hisCount] = his.cs[i].id;
              fdHeader1.jumpList[hisCount++] = his.cs[i].jump + 1;
            }
          }
          fdHeader1.pCount = hisCount;
          fdHeader1.dstPosition = Position;

          p->AddHeader (fdHeader1);
          tHeader = TypeHeader(GPSRTYPE_FD);
          tHeader.srcID = m_neighbors.selfID;
          p->AddHeader (tHeader);      
          Ptr<NetDevice> oif = m_ipv4->GetObject<NetDevice> ();
          Ptr<Ipv4Route> route = Create<Ipv4Route> ();
          route->SetDestination (dst);
          route->SetSource (header.GetSource ());
          route->SetGateway (nextTarget);
  ...
  else if(tHeader.Get() == GPSRTYPE_FD)
  {
    int in = 0;
    for(uint32_t i = 0; i < MAX_CS_COUNT; i++){
      if(tHeader.srcID == m_neighbors.cs[i].connectID){
        in = 1;
        break;
      }
    }
    ...
    FDHeader fdHeader;
    p->RemoveHeader(fdHeader);
    T_Connect his;
    //NS_ASSERT(fdHeader.pCount != 0);
    his.count = fdHeader.pCount;
    
    for(uint32_t i = 0; i < MAX_CS_COUNT; i++){
      his.cs[i].id = NO_CONNECT;
    }
    for(uint32_t i = 0; i < his.count; i++){
      his.cs[i].id = fdHeader.idList[i];
      his.cs[i].jump = fdHeader.jumpList[i];
    }
    ...
    //std::cout << m_neighbors.selfID << "(" << m_neighbors.selfID - 255 << ") in FD, To: " << nextTarget << "\n";


    if(m_neighbors.HasValidPosition(m_neighbors.GetPosition(nextTarget))){
      PositionHeader posHeader (fdHeader.dstPosition.x, fdHeader.dstPosition.y, 0, (uint64_t) 0, (uint64_t) 0, (uint8_t) 0, myPos.x, myPos.y);
      p->AddHeader (posHeader);
    ...
    fdHeader1.pCount = hisCount;
    fdHeader1.dstPosition = fdHeader.dstPosition;
    p->AddHeader (fdHeader1);
    tHeader = TypeHeader(GPSRTYPE_FD);
    tHeader.srcID = m_neighbors.selfID;
    p->AddHeader (tHeader);     
